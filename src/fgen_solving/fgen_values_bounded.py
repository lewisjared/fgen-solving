"""
Python wrapper of the Fortran module ``fgen_values_bounded_w``

``fgen_values_bounded_w`` is itself a wrapper
around the Fortran module ``fgen_values_bounded``.
"""
...
try:
    from fgen_solving._lib import fgen_values_bounded_w  # type: ignore
...
@define
class ValuesBounded(FinalizableWrapperBase):
    """
    Wrapper around the Fortran :class:`ValuesBounded`

    Class for handling values with the idea of bounds included
    """
    ...
    def from_build_args(
        ...
        Alternatively a :obj:`~ValuesBoundedContext`
        ...
        See Also
        --------
        :meth:`ValuesBoundedContext.from_build_args`
        """
        ...
        execute_finalize_on_fail(
            out,
            fgen_values_bounded_w.instance_build,
            values=values_m,
    ...
    @classmethod
    def from_new_connection(cls) -> ValuesBounded:
        """
        ...
        """
        instance_index = fgen_values_bounded_w.get_free_instance_number()
    ...
    def finalize(self) -> None:
        ...
        """
        fgen_values_bounded_w.instance_finalize(self.instance_index)
    ...
    def values(self) -> pint.registry.UnitRegistry.Quantity:
        ...
        (
            ...
            values_dtype,
            values_shape,
        ) = fgen_values_bounded_w.iget_values_shape(*call_args)
        if values_nd == 0:
        ...
        fgen_values_bounded_w.iget_values(*call_args, values=values_array)
    ...
    def value_last_bound(self) -> pint.registry.UnitRegistry.Quantity:
        ...
        #     magnitude_suffix='_m',
        # )
        value_last_bound_raw = fgen_values_bounded_w.iget_value_last_bound(
            self.instance_index,
        )
    ...
    def units(self) -> str:
        ...
        # )
        call_args = (self.instance_index,)
        units_length = fgen_values_bounded_w.iget_units_length(*call_args)
        units_raw = fgen_values_bounded_w.iget_units(
            *call_args,
            n=units_length,
        )
    ...
    def repr(
        ...
        # )
        call_args = (self.instance_index,)
        res_length = fgen_values_bounded_w.i_repr_length(*call_args)
        res_raw = fgen_values_bounded_w.i_repr(
            *call_args,
            n=res_length,
        )
    ...
    def get_bounds(
        ...
        (
            ...
            bounds_dtype,
            bounds_shape,
        ) = fgen_values_bounded_w.i_get_bounds_shape(*call_args)
        if bounds_nd == 0:
            # Nice example of how error handling could work
            # via basic integers or a derived type
            # (raise on Fortran side, check then process on Python side,
        ...
        fgen_values_bounded_w.i_get_bounds(*call_args, bounds=bounds_array)
...
@define
class ValuesBoundedNoSetters(FinalizableWrapperBase):
    """
    Wrapper around the Fortran :class:`ValuesBounded`
    ...
    def from_build_args(
        ...
        The user is responsible for releasing this connection
        using :attr:`~finalize` when it is no longer needed.
        Alternatively a :obj:`~ValuesBoundedNoSettersContext`
        can be used to handle the finalisation using a context manager.
        ...
        -------
            Built (i.e. linked to Fortran and initialised)
            :obj:`ValuesBoundedNoSetters`

        See Also
        --------
        :meth:`ValuesBoundedNoSettersContext.from_build_args`
        """
        # Extract units from the input, make sure they're all the same
        provided_units = {
            name: str(v.units)
    ...
    @classmethod
    def from_new_connection(cls) -> ValuesBoundedNoSetters:
        """
        Initialise from a new connection
...
@define
class ValuesBoundedContext(FinalizableWrapperBaseContext):
    """
    Context manager for :class:`ValuesBounded`
    """
    ...
    def from_build_args(
        ...
        *args: Any,
        **kwargs: Any,
    ) -> ValuesBoundedContext:
        """
        Docstrings to be handled as part of #223
        """
        return cls(
            ValuesBounded.from_build_args(*args, **kwargs),
        )


@define
class ValuesBoundedNoSettersContext(FinalizableWrapperBaseContext):
    """
    Context manager for :class:`ValuesBoundedNoSetters`
    """
    ...
    def from_build_args(
        ...
        *args: Any,
        **kwargs: Any,
    ) -> ValuesBoundedNoSettersContext:
        """
        Docstrings to be handled as part of #223
        """
        return cls(
            ValuesBoundedNoSetters.from_build_args(*args, **kwargs),
        )
