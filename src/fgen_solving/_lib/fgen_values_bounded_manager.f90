!
! Manager for ``fgen_values_bounded``'s ``ValuesBounded`` derived type
!
! In combination with ``fgen_values_bounded_w``,
! this allows the ``ValuesBounded`` derived type
! to be exposed to Python.
!
module fgen_values_bounded_manager
   ...
   type(ValuesBounded), target, dimension(N_INSTANCES) :: instance_array
...
contains
   ...
   subroutine get_instance(instance_index, instance_pointer)
      ...
      type(ValuesBounded), pointer, intent(inout) :: instance_pointer
...
end module fgen_values_bounded_manager
