!
! Manager for ``fgen_time``'s ``TimeAxis`` derived type
!
! In combination with ``fgen_time_w``,
! this allows the ``TimeAxis`` derived type
! to be exposed to Python.
!
module fgen_time_manager
   ...
   type(TimeAxis), target, dimension(N_INSTANCES) :: instance_array
...
contains
   ...
   subroutine get_instance(instance_index, instance_pointer)
      ...
      type(TimeAxis), pointer, intent(inout) :: instance_pointer
