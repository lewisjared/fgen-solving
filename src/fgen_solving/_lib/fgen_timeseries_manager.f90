!
! Manager for ``fgen_timeseries``'s ``Timeseries`` derived type
!
! In combination with ``fgen_timeseries_w``,
! this allows the ``Timeseries`` derived type
! to be exposed to Python.
!
module fgen_timeseries_manager
   ...
   type(Timeseries), target, dimension(N_INSTANCES) :: instance_array
...
contains
   ...
   subroutine get_instance(instance_index, instance_pointer)
      ...
      type(Timeseries), pointer, intent(inout) :: instance_pointer
...
end module fgen_timeseries_manager
